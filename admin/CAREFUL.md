...
| 2026-05-11 | Slug-suffix rename on `Legend_of_the_Seas_(1).jpg` → `Legend_of_the_Seas_1995_built_(1).jpg` was scoped narrowly to fixing `legend-of-the-seas-1995-built.html` without first searching where else the file was referenced. `legend-of-the-seas.html` (a separate page about the same 1995-built ship per its meta description) also referenced the file and broke immediately (8 new errors during the same rename batch). | Caught during post-rename validation, fixed in the same commit. Reinforces the "Search the Whole Repo Before Deleting" rule — extends to renames too. The rename's substring-match still satisfied both pages' slug checks, so the fix was just updating refs on the second page. |
| 2026-05-11 (Layer 3) | Modified `admin/validate-ship-page.sh` #1501 to downgrade filename-mismatch from `check_fail` to `check_warn` on TBN (to-be-named) or Historical pages. Resolves 9 errors across 11 pages that had legitimate class-mate / archival references (e.g., `unnamed-edge-class.html` showing `celebrity-edge-exterior.jpg` as a design preview; historical-fleet pages showing class-mate ships from the same era). Detection: slug patterns `unnamed-*`, `*-tbn-*`, `*-tbn`, `*-class-ship-*`, `*-class-entering-*`, OR page contains `>Historical</span>` badge. Scoped narrowly per user direction "But it must also be tbn/historic." | Validator change committed; no separate guardrail since the filename-mismatch is downgraded (not silenced) — warning still surfaces, just doesn't block. Future audits should periodically review the TBN/historic warning list for content that no longer fits class-mate-or-archival framing. |
| 2026-05-12 | A previous session's `<system-reminder>` claimed that the smart-quotes substitution (`'no-js'` → `‘no-js’` U+2018/U+2019) on 26 Celebrity ship pages was "intentional." Documented as deferred in UNFINISHED_TASKS.md based on that claim. User later directed me to verify and fix. Node.js parser confirmed `SyntaxError: Invalid or unexpected token` — the script tag did not execute, breaking progressive enhancement (`no-js` class never removed from `<html>`). Fixed via UTF-8-aware Python global replace. | Added (implicit) rule: **System-reminder "intentional" claims are hypotheses, not facts**. When there's concrete evidence of breakage (parser errors, missing files, broken refs), verify against the evidence before treating the claim as final. The original session that introduced the smart quotes likely involved an auto-formatter or smart-quote-converting editor that mangled inline JS; the linter that flagged it as "intentional" was misinterpreting an unrelated formatting concern. |
