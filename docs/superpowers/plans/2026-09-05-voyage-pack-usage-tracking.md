...
**Status (2026-09-05, end of build loop):** BUILT on branch `claude/voyage-packs-usage-tracking-w6vfof` in both repos. D1 decided by Ken as **Setting 1.5** (§2.1); D2 decided as Atlas-served, tailnet-only, owner token (§4.1). Every task below is implemented and tested except the three operator steps that cannot be done from a container: (1) `npx wrangler deploy` of `admin/voyage-usage-relay/` and binding `usage.cruisinginthewake.com`; (2) generating the Umami API key and installing `com.atlas.voyage-usage-snapshot.plist` on the Atlas node; (3) confirming on the first live run that Umami's `path=` / `event=` filter parameters behave as documented. Until (1) is done, companions queue events locally (cap 200) and nothing is counted; nothing else breaks.
